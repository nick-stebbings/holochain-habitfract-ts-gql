...
export class HolochainApp extends LitElement {
  ...
  async firstUpdated() {
    ...
    const appInfo = await appWebsocket.appInfo({
      installed_app_id: 'habit_fract',
    });

    const cellData = appInfo.cell_data.find(data => data.role_id === 'habit_lobby') as InstalledCell;

    const result = await appWebsocket.callZome({
      cap: null as any,
      cell_id: cellData.cell_id,
      zome_name: 'habit_tracking',
      fn_name: 'create_domain',
      payload: {
  "name": "sports",
  "hashtag": "#cool"
},
  ...
  render() {
    return html`
      ...
      ${this.entryHash
        ? html`<span
            >Created new Holochain entry! domain with hash: ${this.entryHash}</span
          >`
